...
class DishService:
    ...
    def get_items(self, menu_id: int, submenu_id: int) -> list[dict]:
        dish_redis = DishRedis.get_dishes(menu_id, submenu_id)
        if dish_redis:
            return pickle.loads(dish_redis)
        dishes = self.dish_repository.get_dishes(menu_id, submenu_id)
        result = []
        for dish in dishes:
            temp = {
                'id': str(dish.id),
            ...
            result.append(temp)
        DishRedis.set_dishes(menu_id, submenu_id, pickle.dumps(result))
        return result

    def create_item(self, menu_id: int, submenu_id: int, dish_data: schemas.Dish) -> JSONResponse:
    ...
    def get_item(self, menu_id: int, submenu_id: int, dish_id: int) -> JSONResponse:
    ...
    def update_item(self, menu_id: int, submenu_id: int, dish_id: int, dish_data: schemas.Dish) -> JSONResponse:
    ...
    def delete_item(self, menu_id: int, submenu_id: int, dish_id: int) -> JSONResponse:
