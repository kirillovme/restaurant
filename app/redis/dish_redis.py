...
class DishRedis:
    ...
    @classmethod
    def set_dishes(cls, menu_id: int, submenu_id: int, data: bytes) -> None:
        cls.redis_connect.set('menu__' + str(menu_id) + '__submenus__' + str(submenu_id) + '__dishes', data, get=True)

    @classmethod
    def get_dishes(cls, menu_id: int, submenu_id: int) -> bytes | None:
        return cls.redis_connect.get('menu__' + str(menu_id) + '__submenus__' + str(submenu_id) + '__dishes')

    @classmethod
    def delete_dishes(cls, menu_id: int, submenu_id: int) -> None:
        cls.redis_connect.delete('menu__' + str(menu_id) + '__submenus__' + str(submenu_id) + '__dishes')

    @classmethod
    def set_dish(cls, menu_id: int, submenu_id: int, dish_id: int, data: bytes) -> None:
        cls.redis_connect.\
            set('menu__' + str(menu_id) + '__submenus__' + str(submenu_id) + '__dishes__' + str(dish_id), data)

    @classmethod
    def get_dish(cls, menu_id: int, submenu_id: int, dish_id: int) -> bytes | None:
        return cls.redis_connect.\
            get('menu__' + str(menu_id) + '__submenus__' + str(submenu_id) + '__dishes__' + str(dish_id))

    @classmethod
    def delete_dish(cls, menu_id: int, submenu_id: int, dish_id: int) -> None:
        cls.redis_connect.\
            delete('menu__' + str(menu_id) + '__submenus__' + str(submenu_id) + '__dishes__' + str(dish_id))

    @classmethod
    def clear_dish_data(cls, menu_id: int, submenu_id: int, dish_id: int) -> None:
